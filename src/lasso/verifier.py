...
class Verifier:
    ...
    def verify(self, pf: Proof) -> bool:
        ...
        self.verify_sumcheck_data(sumcheck_h_data, a_eval)
        ...
        # Todo: check h_eval = eq(r, rz) * self.table.g_func(E_eval)
        ...
        for i in range(self.alpha):
            self.verify_grand_product_PIOP(S_data[i], S_comm[i], r_prime2)
            self.verify_grand_product_PIOP(RS_data[i], RS_comm[i], r_prime3)
            self.verify_grand_product_PIOP(WS1_data[i], WS1_comm[i], r_prime4)
            self.verify_grand_product_PIOP(WS2_data[i], WS1_comm[i], r_prime5)
            ...
            # assert(S_data[i].f_0_r == Hash((Scalar(), self.table.tables[i][], final_cts_eval[i]), tau, gamma))
