...
class Verifier:
    ...
    def verify(self, pf: Proof) -> bool:
        ...
        for i in range(self.alpha):
            ...
            assert(self.setup.verify(E_comm[i], r_prime3, E_eval2[i], E_eval2_proof[i]))
            assert(self.setup.verify(dim_comm[i//self.k], r_prime3, dim_eval, dim_eval_proof[i]))
            assert(self.setup.verify(read_ts_comm, r_prime3, read_ts_eval, read_ts_eval_proof[i]))
            assert(self.setup.verify(final_cts_comm, r_prime2, final_cts_eval, final_cts_eval_proof[i]))
