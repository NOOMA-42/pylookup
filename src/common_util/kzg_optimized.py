...
from src.common_util.curve_optimized import ec_lincomb, G1Point, G2Point, Scalar, G1, G2, ec_pairing
...
class KZGSetup(object):
    ...
    def open(self, poly: Polynomial, x: Scalar) -> (Scalar, G1Point):
        ...
        if poly.basis == Basis.LAGRANGE:
            poly = poly.ifft()
        q = (poly - v) / Polynomial([Scalar(-x), 1], Basis.MONOMIAL)
        return v, self.commit_G1(q)

    def verify(self, C: G1Point, pi: G1Point, x: Scalar, v: Scalar):
        LHS = ec_pairing(G2, C - v)
        comm = self.commit_G2(Polynomial([Scalar(-x), 1], Basis.MONOMIAL))
